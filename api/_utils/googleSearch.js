 ...
 *    - OpenAlex API key (hardcoded — see OPENALEX_NOTE below)
 ...
 * The OpenAlex API key below is intentionally hardcoded because the key was

 * provisioned for this project. OpenAlex accepts the key as a query parameter

 * (api_key=...) and as a mailto contact for the "polite pool".

 *

 * Resolution order at request time:

 *   1. Hardcoded OPENALEX_API_KEY (below)

 *   2. process.env.OPENALEX_API_KEY (server override)

 *   3. Caller-supplied openAlexKey in the search() call signature

 *

 * If you rotate the key, update OPENALEX_API_KEY here AND remove the old one

 * from any cached server env vars.

 * ─────────────────────────────────────────────────────────────────────────── */

const OPENALEX_API_KEY = 'czY4nJLBN0j19bNlVb2GXn';


function resolveOpenAlexKey(override = null) {

    return override || process.env.OPENALEX_API_KEY || OPENALEX_API_KEY;
...
export const GoogleSearchAPI = {
     ...
     * @param {string}  query      Essay text or topic string

     * @param {string}  apiKey     Google Custom Search API key (kept for parity)

     * @param {string}  cx         Google Custom Search engine ID

     * @param {string}  groqKey    Groq API key (optional — kills Stage 1 + Stage 5 if missing)

     * @param {string}  openAlexKey Optional OpenAlex key override; falls back to env / hardcoded
    ...
    async search(query, apiKey, cx, groqKey = null, openAlexKey = null) {
        ...
        // Resolve the OpenAlex key once per search so every parallel request uses it

        const openAlexKeyResolved = resolveOpenAlexKey(openAlexKey);


        // Stage 3: Fetch from OpenAlex

        const openAlexResults = await this._searchOpenAlex(queries, openAlexKeyResolved, stats);
        ...
        // Record which key was actually used (masked) — helps debugging key drift

        stats.openAlexKeySource = openAlexKey
...
Return ONLY the raw JSON object, no markdown.`;
     ...
     * Fetches papers from OpenAlex API in parallel. The API key is appended as

     * `&api_key=...` so OpenAlex routes us into the faster "polite pool" tier.

     *

     * @param {string[]} queries     Search phrases generated from the Search Brief

     * @param {string}   openAlexKey Resolved OpenAlex API key

     * @param {object}   stats       Stats tracker (mutated in place)

     */

    async _searchOpenAlex(queries, openAlexKey, stats) {
        ...
        await Promise.all(queries.map(async (query) => {
            ...
            try {
                ...
                // OpenAlex expects api_key as a query param + a mailto for polite pool
                ...
                const url = `https://api.openalex.org/works` +
                            ...
                            `&api_key=${encodeURIComponent(openAlexKey)}` +
                ...
                const res = await fetch(url, {
                    ...
                    headers: {
                        ...
                        // OpenAlex accepts the key in the Authorization header too,

                        // keeping both code paths makes it work even if proxies strip query params

                        'Authorization': `Bearer ${openAlexKey}`
...
Return ONLY a raw JSON array of index numbers, e.g.: [0, 1, 3, 5]`;
    ...
    _createStats() {
        ...
        return {
            ...
            openAlexKeySource: 'unresolved', // 'caller' | 'env' | 'hardcoded'
